...
VERSION=$(curl -sSL https://api.github.com/repos/Droid-MAX/aliyun-ddns-cli/commits/master | sed -n '{/sha/p; /date/p;}'| sed 's/.* \"//g' | cut -c1-10 | tr '[:lower:]' '[:upper:]' | sed 'N;s/\n/@/g' | head -1)
...
tar -C ./release -zcf ./release/${name}_arm-$VERSION.tar.gz $(for v in ${ARMS[@]}; do echo -n "./${name}_arm$v ";done) ${name}_arm64
...
$MD5 ./release/${name}_mips-$VERSION.tar.gz
...
$MD5 ./release/${name}_mipsle-sf-$VERSION.tar.gz
...
$MD5 ./release/${name}_mips-sf-$VERSION.tar.gz
